    ...
    <div className="min-h-screen bg-white p-4 sm:p-6 lg:p-6">
      ...
      <header className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <Link to="/users/create" className="flex items-center justify-center gap-2 p-3 rounded-lg border border-gray-200 bg-white/90 text-gray-800 font-medium text-sm hover:bg-blue-50 transition-all duration-200" aria-label="Create user">
              ...
              <>
                ...
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border border-gray-200 z-20">
                  <div className="py-2">
                    ...
                    {columns.map(col => (
                      <label key={col.key} className="flex items-center cursor-pointer px-4 py-2 hover:bg-gray-50">
                        <input type="checkbox" checked={col.visible} onChange={() => toggleColumn(col.key)} className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 focus:ring-2" />
              ...
              <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                ...
                {columns.find(c => c.key === 'status')?.visible && (
                  <td className="px-4 sm:px-6 py-3 text-left">
                    {u.isValidated
                      ? <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="green" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75" /></svg>
                  ...
                  <td className="px-4 sm:px-6 py-3 relative text-right">
                    ...
                    {openMenuId === u.id && (
                      <div className="absolute right-2 bg-white border border-gray-200 shadow-lg rounded-lg z-50 min-w-28 w-max">
                        {!u.isValidated && <button onClick={() => handleValidate(u.id)} className="block w-full text-left px-4 cursor-pointer py-2 text-xs sm:text-sm hover:bg-green-50 text-gray-700">Validate</button>}
                        <button className="block w-full text-left px-4 cursor-pointer py-2 text-xs sm:text-sm hover:bg-green-50 text-gray-700" onClick={() => document.getElementById('modal_'+u.username)?.showModal()}>Update password</button>
                        <button onClick={() => handleDelete(u.id)} className="block w-full text-left px-4 cursor-pointer py-2 text-xs sm:text-sm hover:bg-red-50 text-red-600">Block</button>
                      </div>
                    )}
                  </td>
                )}
      ...
      {/* Modales */}
      {data?.map(u => <UpdatePassword key={`modal-${u.id}`} u={u} />)}
    </div>
  );
};
