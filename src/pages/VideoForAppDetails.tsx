...
const VideoForAppDetails: React.FC = () => {
  ...
  const [platform, setPlatform] = useState<{ id: number; name: string } | null>(
    null,
  );

  const selectableUsers = users.filter((u: any) => {
    const isAdmin = u.role === "admin" && !u.isDeleted;
    const hasAccess = video?.accesses?.some(
      (access: any) => access.target_user_id === u.id,
    );
  ...
  const videoTitles: MangaTitles = video
    ? [
        ...(video.cn_title
          ? [
              {
                i18_language: "zh" as any,
                title: video.cn_title,
                description: "",
              },
            ]
          : []),
        ...
        ...(video.hi_title
          ? [
              {
                i18_language: "hi" as any,
                title: video.hi_title,
                description: "",
              },
            ]
          : []),
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={async () => {
                          ...
                          if (!should) return;
                          try {
                            await updateVideoForAppBannedStatus(
                              video.id,
                              !video.isBanned,
                            );
                            toast.success(
                              `视频 ${!video.isBanned ? "已封禁" : "已解封"} 成功`,
                            );
                            reFetch();
                          } catch (error: any) {
                            toast.error(
                  ...
                  <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-4">
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        {getCategoryDisplayName(video.categories)}
                      </div>
                      <div>
                        <span className="font-medium">子分类:</span>{" "}
                        {getSubCategoryDisplayName(video.sub_categories)}
                      </div>
                      <div>
                        <span className="font-medium">平台:</span>{" "}
                        {getPlatformDisplayName(video.plateform)}
                      </div>
                      <div>
                        <span className="font-medium">视频类型:</span>{" "}
                        {video.type === "1" ? (
                          <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300 ml-1">
                            短视频
                          </span>
                        ) : (
                          <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 ml-1">
                            长视频
                          </span>
                        )}
                      </div>
                      <div>
                        ...
                        {video.need_vip ? (
                          <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300 ml-1">
                            VIP
                          </span>
                        ) : (
                          <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-gray-100 text-gray-500 dark:bg-gray-800/30 dark:text-gray-400 ml-1">
                            非VIP
                          </span>
                        )}
                      </div>
