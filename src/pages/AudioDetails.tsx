...
const AudioDetails: React.FC = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center px-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-1">Audio introuvable</h2>
          <Link to="/audios" className="text-sm text-gray-600 dark:text-gray-300 underline">Retour à la liste</Link>
        </motion.div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="max-w-5xl mx-auto px-3 py-6">
        ...
        <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="mb-4">
          <Link to="/audios" className="inline-flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 hover:text-gray-900">
  ...
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-md p-3 shadow sticky top-6">
              ...
              <div className="relative w-full h-40 rounded-md overflow-hidden bg-gray-100 dark:bg-gray-900 mb-3">
                {audio.cover_url || audio.s3_cover_url ? (
                  <img src={audio.cover_url || audio.s3_cover_url} alt={audio.title} className="w-full h-full object-cover" />
                ) : (
                ...
                {audio.need_vip && (
                  <div className="absolute top-2 right-2 px-2 py-0.5 bg-yellow-400 text-white rounded text-xs font-semibold">VIP</div>
                )}
              </div>
              ...
              {/* Compact status */}
              <div className="mb-3 flex items-center justify-center">{getCheckingBadge(audio.checking)}</div>

              {/* Actions: compact buttons */}
              <div className="flex gap-2">
                <Link to={`/audios/${audio.id}/edit`} className="flex-1 inline-flex items-center justify-center gap-2 px-2 py-2 bg-gray-100 dark:bg-gray-900 text-sm rounded text-gray-700 dark:text-gray-200 hover:bg-gray-200">
                ...
                {user?.role === RoleEnum.SUPERADMIN && (
                  <>
                    <button onClick={handleSendToS3} disabled={uploading} className="inline-flex items-center gap-2 px-2 py-2 bg-gray-100 dark:bg-gray-900 text-sm rounded text-gray-700 hover:bg-gray-200 disabled:opacity-50">
                      <Send className="w-4 h-4" />
                    </button>
                    <button onClick={handleToggleDelete} className={`inline-flex items-center gap-2 px-2 py-2 text-sm rounded ${audio.isDeleted ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
                      {audio.isDeleted ? <CheckCircle className="w-4 h-4" /> : <Trash2 className="w-4 h-4" />}
                    </button>
                  </>
                )}
              </div>
          ...
          <motion.div initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.12 }} className="lg:col-span-2 space-y-3">
            <div className="bg-white dark:bg-gray-800 rounded-md p-4 shadow">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <h1 className="text-lg font-semibold text-gray-900 dark:text-gray-100 truncate">{audio.title}</h1>
                  {audio.ref && <div className="text-xs text-gray-500 dark:text-gray-400">Réf. {audio.ref}</div>}
                </div>
                <div className="text-right text-xs text-gray-500 dark:text-gray-400">{formatDateFR(audio.createdAt)}</div>
              </div>

              {audio.description && <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">{audio.description}</p>}

              <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
                <div className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900/40 rounded">
                  <Clock className="w-4 h-4 text-gray-600" />
                  <div className="text-xs text-gray-700 dark:text-gray-300">{formatDuration(audio.duration)}</div>
                </div>

                {audio.audioCategory && (
                  <div className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900/40 rounded">
                    <Tag className="w-4 h-4 text-gray-600" />
                    <div className="text-xs text-gray-700 dark:text-gray-300">{audio.audioCategory.name}{audio.audioSubCategory ? ` › ${audio.audioSubCategory.name}` : ""}</div>
                  </div>
                )}
                ...
                {(audio.creator || audio.creatorObj) && (
                  <div className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900/40 rounded">
                    ...
                    <div className="text-xs text-gray-700 dark:text-gray-300">{audio.creator || audio.creatorObj?.name}</div>
                  </div>
                )}
                ...
                {audio.plateform && (
                  <div className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900/40 rounded">
                    ...
                    <div className="text-xs text-gray-700 dark:text-gray-300">{audio.plateform.name}</div>
                  </div>
                )}
                ...
                {audio.upload_status && (
                  <div className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900/40 rounded">
                    ...
                    <div className="text-xs text-gray-700 dark:text-gray-300 capitalize">{audio.upload_status}</div>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-md p-3 shadow">
                ...
                <div className="flex flex-wrap gap-2">
                  {audio.tagCategories.map((tag) => (
                    <span key={tag.id} className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-900/30 rounded">{tag.name}</span>
                  ))}
                </div>
              </div>
            )}
