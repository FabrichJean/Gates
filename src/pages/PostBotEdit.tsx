...
import { type Image, type Video } from "../hooks/usePost";
...
const PostBotEdit = () => {
    ...
    const { data: post_bot, loading, error } = UsePostBot(id);
    ...
    useEffect(() => {
        if (post_bot) {
            setMedia({
                images: post_bot.images || [],
                videos: post_bot.videos || [],
            });
        }
    }, [post_bot]);
    ...
    useEffect(() => {
        if (post_bot) {
            const matchingCategory = categoriesResponse?.categories.find(
                (cat) => cat.id === post_bot.postCategory.id
            );
            ...
            if (post_bot.titles && post_bot.titles.length > 0) {
                ...
                post_bot.titles.forEach((item, index) => {
            ...
            if ((post_bot as any).creatorObj) {
                setCreatorObj((post_bot as any).creatorObj || null);
            }

            // initialize selected tags from post.tagCategory
            if (Array.isArray((post_bot as any).tagCategory)) {
                setSelectedPostTagCategories(
                    ((post_bot as any).tagCategory as any[]).map((t) => ({
                        id: t?.id,
                        name: t?.name,
                        meta: t?.meta ?? null,
                    }))
    ...
    const uploadExistingCover = async () => {
        if (!pendingCover || !post_bot) return;
        const { videoId, file } = pendingCover;
        setUploadingCover(true);
        try {
            const url = `${apiURL}/posts/${post_bot.id}/images`;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload = {
            id: post_bot?.id,
            category_id: selectedCategory?.id ?? post_bot?.postCategory?.id,
            sub_category_id: selectedSubCategory?.id ?? post_bot?.postSubCategory?.id,
        ...
        try {
            ...
            if (hasNewFiles) {
                ...
                if (post_bot?.id) fd.append("id", String(post_bot.id));
                fd.append(
                    "category_id",
                    String(selectedCategory?.id ?? post_bot?.postCategory?.id ?? "")
                );
                fd.append(
                    "sub_category_id",
                    String(selectedSubCategory?.id ?? post_bot?.postSubCategory?.id ?? "")
                );
                ...
                await updatePost(post_bot?.id, fd);
            } else {
                ...
                await updatePost(post_bot?.id, payload);
            }
                ...
                <div className="w-full">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">
                            Modify: POST-{String(post_bot.id).padStart(3, "0")}
                        </h2>
                        <button
