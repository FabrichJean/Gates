...
import {
  getPostsForApp,
  sendPostsForAppToWebApp,
  togglePostForAppStatus,
} from "../api/postsForApp";
...
export const usePostForAppManagement = () => {
  ...
  // Load initial filters from localStorage synchronously
  ...
  const [filters, _setFilters] = useState(getInitialFilters());
  ...
  // No need for separate localStorage filter effect, handled in initial state
  ...
  const fetch = useCallback(
    async (pageNumber = 1, customFilters = null) => {
      try {
        setLoading(true);
        ...
        const normalizedFilters = {
          ...filters,
          ...(customFilters || {}),
          isDeleted: normalizeBool(
            customFilters && customFilters.isDeleted !== undefined
        ...
        const params = Object.fromEntries(
          Object.entries({ ...normalizedFilters, page: pageNumber }).filter(
            ([k, v]) => v !== "all",
          ),
        );
  ...
  const activate = async (id: number) => {
    try {
      await togglePostForAppStatus(id);
