...
import { getPostsBot, sendPostsToWebApp, togglePostStatus } from "../api/posts";
import type { PostsResponse } from "../types/post";
import toast from "react-hot-toast";

export const usePostBotManagement = () => {
  ...
  const fetch = useCallback(async (pageNumber = 1) => {
    try {
      ...
      const res = await getPostsBot({ page: pageNumber });
...
export default usePostBotManagement;
