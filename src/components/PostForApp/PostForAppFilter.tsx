...
export default function PostForAppFilter({
    ...
    const { data: categoriesResponse, loading: categoriesLoading, error: categoriesError } = useCategoryPostForApp();
    const { data: subcat } = useSubCategoryPostForApp(Number(filters?.category_id));
    ...
    useEffect(() => {
        try {
            ...
            console.warn("⚠️ Impossible de lire le filtre sauvegardé posts for app:", e);
    ...
    const submit = async () => {
        ...
        localStorage.setItem("posts_for_app_filtered", JSON.stringify({ ...filters }));

        try {
            const fetched = await getPostsForApp(data);
            onSubmit(fetched.data);
        } catch (error) {
            console.error("Erreur lors du filtrage posts for app:", error);
        }
    };
