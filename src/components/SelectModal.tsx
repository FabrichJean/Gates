...
interface SelectModalProps {
    open: boolean;
    onClose: () => void;
    onSubmit?: (optionId: string | null, label: number | null) => void;
    title?: string;
    options?: Option[];
    rowLabel?: number | null;
}

const SelectModal = ({ open, onClose, onSubmit, title = "Sync option", options = [], rowLabel = null }: SelectModalProps) => {
    const [selected, setSelected] = useState<string | null>(options.length ? options[0].id : null);
            ...
            <div className="relative p-4 w-full max-w-md max-h-full inset-0 dark:bg-gray-800 backdrop-blur-2xl border boder-white shadow-lg">
                <div className="relative bg-neutral-primary-soft border border-default rounded-base shadow-sm p-4 md:p-6">
                    ...
                    <div className="pt-4 md:pt-6">
                        {rowLabel && (
                            <p className="text-sm text-body mb-3">Selected row id: <strong className="text-heading">{rowLabel}</strong></p>
                        )}
                                ...
                                <li key={opt.id}>
                                    ...
                                    <label htmlFor={opt.id} className="inline-flex items-center w-full p-5 text-body bg-neutral-primary-soft border-1 border-default rounded-base cursor-pointer peer-checked:hover:bg-brand-softer peer-checked:border-brand-subtle peer-checked:bg-brand-softer hover:bg-neutral-secondary-medium peer-checked:text-fg-brand-strong">
                                        ...
                                        <div className="block ms-2.5">
                                            <div className="w-full text-base font-medium">{opt.title}</div>
                                            {opt.subtitle && <div className="w-full text-xs">{opt.subtitle}</div>}
                                        </div>
                                        <svg className="w-5 h-5  rtl:rotate-180 ms-auto" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 12H5m14 0-4 4m4-4-4-4"/></svg>
                                    </label>
                                </li>
                            ))}
                        </ul>
