...
interface Props {
  onSelect?: (category: RomanCategory) => void;
  defaultValue?: RomanCategory;
}

const RomanCategoryAutoComplete = ({ onSelect, defaultValue }: Props) => {
  const [categories, setCategories] = useState<RomanCategory[]>([]);
  const [query, setQuery] = useState(defaultValue?.name || "");
  const [filtered, setFiltered] = useState<RomanCategory[]>([]);
  const [showDropdown, setShowDropdown] = useState(false);

  // Référence pour détecter le clic à l'extérieur
  const dropdownRef = useRef<HTMLDivElement>(null);

  // Charger toutes les catégories de romans
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await axios.get<RomanCategory[]>(`${apiURL}/roman/category/all`, {
          headers: { Authorization: `Bearer ${getToken()}` },
        });
        setCategories(res.data || []);
        setFiltered(res.data || []);
      } catch (error) {
        console.error("Erreur lors du chargement des catégories de romans :", error);
      }
    };
  ...
  const handleSelect = (category: RomanCategory) => {
...
export default RomanCategoryAutoComplete;
