...
export type UploadChoice = "simple" | "double";
			...
			<div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
				...
				<div className="relative z-10 inline-block align-bottom bg-white dark:bg-gray-900 rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
					...
					<div className="mt-4 space-y-3">
						<label
							className={`flex items-center gap-3 rounded-md border px-3 py-2 cursor-pointer transition-colors ${choice === "simple"
						...
						>
							<input
								...
								name="upload-mode"
								value="simple"
								checked={choice === "simple"}
								onChange={() => onChoiceChange("simple")}
								className="h-4 w-4 appearance-auto accent-blue-600"
							/>
							<span className="text-sm font-medium text-gray-900 dark:text-gray-100">
								{t("videos.upload.confirm.simple")}
							</span>
						</label>

						<label
							className={`flex items-center gap-3 rounded-md border px-3 py-2 cursor-pointer transition-colors ${choice === "double"
								? "border-blue-500 bg-blue-50 dark:bg-blue-900/30"
								: "border-gray-200 dark:border-gray-700"
								}`}
						>
							<input
								...
								name="upload-mode"
								value="double"
								checked={choice === "double"}
								onChange={() => onChoiceChange("double")}
								className="h-4 w-4 appearance-auto accent-blue-600"
							/>
							<span className="text-sm font-medium text-gray-900 dark:text-gray-100">
								{t("videos.upload.confirm.double")}
							</span>
						</label>
					</div>
					...
					<div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
						<span className="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
							...
							>
								{uploading ? t("common.processing") : t("videos.upload.confirm.confirm")}
							</button>
						</span>
